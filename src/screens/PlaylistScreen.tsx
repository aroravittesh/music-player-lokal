...
import { getPlaylist, type PlaylistDetail } from "../api/saavn";
...
type PlaylistRoute = RouteProp<
  { Playlist: { playlistId: string; title?: string; imageUrl?: string } },
  "Playlist"
>;
...
export default function PlaylistScreen() {
  ...
  const { playlistId, title: paramTitle, imageUrl: paramImage } = route.params;
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      title: detail?.name ?? paramTitle ?? "Playlist",
    });
  ...
  const playAll = useCallback(() => {
    ...
    setQueue(songs);
    setSong(songs[0], 0);
  }, [detail?.songs, playlistId, setPlaybackContext, setQueue, setSong]);
  ...
  const playTrack = useCallback(
    ...
    [detail?.songs, playNowKeepQueue, playlistId, setPlaybackContext]
  );

  const shuffleOnHere =
    playbackContext?.kind === "playlist" &&
    playbackContext.id === playlistId &&
    shuffleEnabled;
    ...
    return (
      <View style={[styles.centered, { paddingBottom: insets.bottom }]}>
        <View style={styles.loadingColumn}>
          ...
          <Text style={styles.loadingHint}>Loading playlist…</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={styles.screenRoot}>
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? (
            <Text style={styles.emptyTracks}>No songs in this playlist.</Text>
          ) : null
        }
